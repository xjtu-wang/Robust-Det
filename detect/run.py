...
def load_base_model_and_tokenizer(name):
    if args.openai_model is None:
        ...
        base_model_kwargs = {}
        if 'gpt-j' in name or 'neox' in name or 'Llama2' in name:
            base_model_kwargs.update(dict(torch_dtype=torch.float16))
        if 'gpt-j' in name:
            base_model_kwargs.update(dict(revision='float16'))
        if name=='gpt-j':
            name='EleutherAI/gpt-j-6b'
        if name=='Llama2-7b-hf':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack_args', 
                        type=str, default='_L0O0')
    parser.add_argument('--reverse_pred', 
                        action='store_true')
    args = parser.parse_args()
    args.openai_key = "your/openai/key"
    ...
    print("***Currently Running run.py ***")
    ...
    XLSX_PATH = f"logs/{args.generate_model_name}/{args.generate_model_name}@{args.base_model_name.replace('/', '-')}_{args.attack_method}{args.attack_args if args.attack_args!='' else 'None'}.xlsx"
    ...
    if not args.skip_baselines:
        ...
        if os.path.exists(f'models/microsoft_deberta-v3-large@D_{args.generate_model_name}@G.pt'):
            baseline_outputs.append(eval_supervised(data, model=f'models/microsoft_deberta-v3-large@D_{args.generate_model_name}@G.pt'))
        else:
            print("skip fine-tuned deberta because can't find the model file.")
    ...
    # outputs = [{'name':, 'prediction':, 'info':, 'raw_results':, 'metrics':, 'pr_metrics':, }]
